...
;(function () {
  ...
  const ZenoSelect = {
    init() {
      $$(".select-wrapper:not([data-multiselect])").forEach(wrapper => {
        ...
        if (!select.textContent.trim() || select.querySelector(".select-placeholder")) {
          ...
          if (selectedValues.length === 0) {
            ...
            $$(".multiselect-tag-remove", multiselect).forEach(btn => {
              btn.addEventListener("click", e => {
                ...
                const option = $(`.select-option[data-value="${value}"]`, dropdown)
  ...
  const ZenoDualRange = {
    init() {
      $$(".dual-range").forEach(wrapper => {
        ...
        const minDisplay = inputGroup ? $(".dual-range-min", inputGroup) : null
        const maxDisplay = inputGroup ? $(".dual-range-max", inputGroup) : null
        ...
        items.forEach(item => {
          item.addEventListener("click", () => {
            ...
            const event = new CustomEvent("tabbar-change", {
              detail: {
                value: item.dataset.value || item.querySelector("span")?.textContent
              }
            })
    ...
    calculate(triggerEl, floatingEl, preferredPosition = "top", offset = 8) {
      ...
      switch (position) {
        case "top":
          ...
          coords.left = triggerRect.left + triggerRect.width / 2 - floatingRect.width / 2
          break
        case "bottom":
          ...
          break
        case "left":
          coords.top = triggerRect.top + triggerRect.height / 2 - floatingRect.height / 2
  ...
  const ZenoTooltip = {
    ...
    attach(trigger) {
      const show = () => {
        ...
        if (this.currentTrigger === trigger && this.tooltip.style.display !== "none") {
          return
        }
  ...
  const ZenoPopover = {
    ...
    open(triggerId, popoverId) {
      const trigger = typeof triggerId === "string" ? $(`#${triggerId}`) : triggerId
      const popover = typeof popoverId === "string" ? $(`#${popoverId}`) : popoverId
  ...
  const ZenoDatepicker = {
    ...
    attach(wrapper) {
      ...
      const config = {
        ...
        minDate: wrapper.dataset.minDate ? new Date(wrapper.dataset.minDate) : null,
        maxDate: wrapper.dataset.maxDate ? new Date(wrapper.dataset.maxDate) : null,
      ...
      // State
      let selectedDate = wrapper.dataset.value ? new Date(wrapper.dataset.value) : null
      ...
      const renderDays = () => {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          weekdayHeaders.push(
            `<div class="datepicker-weekday">${dayNames[dayIndex].slice(0, 2)}</div>`
          )
        }
      ...
      const renderYears = () => {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          years.push(
            `<button class="${classes.join(" ")}" data-year="${year}">${year}</button>`
          )
        }
    ...
    attach(wrapper) {
      ...
      $$("th.sortable", table).forEach(th => {
        th.addEventListener("click", () => {
          ...
          const isAsc = th.classList.contains("sorted") && !th.classList.contains("desc")
        ...
        $$(".timepicker-option", dropdown).forEach(opt => {
          opt.addEventListener("click", e => {
            ...
            if (opt.dataset.hour !== undefined) {
              ...
              $$("[data-hour]", dropdown).forEach(o => o.classList.remove("selected"))
              opt.classList.add("selected")
            }

            if (opt.dataset.minute !== undefined) {
              selectedMinute = parseInt(opt.dataset.minute)
              $$("[data-minute]", dropdown).forEach(o => o.classList.remove("selected"))
              opt.classList.add("selected")
            }
  ...
  const ZenoStats = {
    ...
    formatNumber(value, options = {}) {
      const {decimals = 0, thousandSeparator = ",", prefix = "", suffix = ""} = options
    ...
    animateValue(element, start, end, duration = 1000, options = {}) {
      ...
      const {formatNumber: formatFn = this.formatNumber, ...formatOptions} = options
  ...
  const ZenoCopyToClipboard = {
    init() {
      // Initialize copy buttons (with data-copy or data-copy-target)
      $$("[data-copy], [data-copy-target]").forEach(button => {
        this.attach(button)
      })
